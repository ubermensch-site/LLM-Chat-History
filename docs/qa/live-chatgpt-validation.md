...
This checklist validates the unpacked extension produced by CI against a real logged-in ChatGPT session. It is intentionally separate from fixture/unit coverage because provider DOM, SPA navigation, streaming, and virtualization are external runtime dependencies.
...
4. Open Library and export JSON.
...
2. Click **Pause**.
3. Send a clearly identifiable placeholder prompt and receive an answer while paused.
4. Click **Resume**.
5. Send another placeholder turn.
6. Open Library and export Markdown/JSON.
...
1. Click **Stop**.
2. Add a disposable placeholder turn.
3. Navigate away and back or refresh.
4. Confirm state remains stopped.
5. Explicitly click **Start** and add a new turn.
...
## Scenario 7 — Minimize semantics

1. While recording, minimize the recorder panel.
2. Add a disposable turn.
3. Reopen the pill and Library.

Pass criteria:

- minimize changes UI only;
- recording never stops or pauses;
- turn is captured normally.
...
## Scenario 9 — Long-thread virtualization

Use a long existing thread if available.

1. Record the number of turns currently rendered near the bottom.
2. Scroll toward the top and back down.
3. Observe whether ChatGPT unmounts older DOM nodes.
4. Confirm previously captured archive messages remain even when nodes disappear.

Pass criteria for current continuous recorder:

- archived turns are never deleted because ChatGPT unmounts them;
- revisiting rendered turns does not create duplicates.

Important: explicit first-time historical scroll-and-harvest import is still TASK-012 follow-up work. Do not mark that capability complete merely because current rendered turns record correctly.
...
4. Download Markdown.
5. Download JSON.
6. Click the browser-extension toolbar icon and confirm it also opens the Library.
...
- `.md` is readable and includes metadata/state boundaries;
...
For TASK-012, capture counts/presence only; never commit private message text.
...
The ChatGPT recorder must not be called production-ready until scenarios 1–8 and 10 pass on a real logged-in browser session. Scenario 9 continuous-capture behavior must also pass; historical import may remain separately tracked if clearly disclosed.
